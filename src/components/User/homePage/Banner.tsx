    ...
    <div className="relative w-full flex flex-col lg:flex-row lg:h-[calc(100vh-3rem)] text-left">
      ...
      >
        <img
          src={banner}
          alt="banner"
          className="w-full lg:h-[calc(100vh-3rem)] object-cover"
        />
      </motion.div>
      ...
      {showText && (
  <motion.div
    className="flex flex-col absolute bottom-0 left-0 w-full p-5 sm:p-10 items-start gap-2"
    initial="initial"
    animate="animate"
    variants={textAnimation}
  >
    <h1 className="text-2xl w-36 sm:w-auto sm:text-3xl lg:text-4xl xl:text-5xl font-bold sm:mb-4 font-oswald bg-custom-gradient-white bg-clip-text text-transparent">
      Welcome to{" "}
      <span className="bg-custom-gradient-blue bg-clip-text text-transparent border-">
        WorkzPro
      </span>
    </h1>
    <p className="hidden sm:block text-sm sm:text-base lg:text-lg xl:text-lg sm:mb-6 text-gray-400 max-w-[12rem] sm:max-w-lg">
